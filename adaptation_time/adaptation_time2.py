...
# Jump near KD/sqrt{alpha}
D_list = np.logspace(-2, 2,15)
C_i = 0.8*KD/np.sqrt(alpha)
C_f =  KD/np.sqrt(alpha) #initial and final (mean) ligand concentrations
...
for j,D in enumerate(D_list):
    ...
    p, cov = opt.curve_fit(linear_prob_bound, theta, f_b)
    A = p[0]; B = p[1]
    FI_end1.append(n * B**2 / (2 * (A - A**2)))
    
    G_end.append(G_next_act)
    FI_time1.append(FI_time_j)
    f_bound_list1.append(f_b)
    f_bound_fit1.append(linear_prob_bound(theta, A, B))
    print(f'{j+1}/{len(D_list)}, D = {D:.4f}. Fisher Information at end = {FI_end1[j]}')


np.savez("raw_data2.npz", dlist=D_list, fitime1=FI_time1, fiend1=FI_end1)
